...
function getDay(val) {
	...
	<body>
		<div class="header">
		<ul>
			<li style="float:left;border-right:none"><a href="ulogin.php" class="logo"><strong> Good Health </strong>Appointment Booking System</a></li>
	...
	<div class="sucontainer" >
		...
		<input type="radio" name="gender" value="" alue="other">Other<br><br>
		...
		<?php
		$sql1="SELECT distinct(address) FROM clinic";
         $results=$conn->query($sql1); 
		while($rs=$results->fetch_assoc()) { 
		?>
		<option value="<?php echo $rs["address"]; ?>"><?php echo $rs["address"]; ?></option>
		<?php
		}
		?>
		</select><br>
		...
		<?php
		$sql1="SELECT distinct(name) FROM doctor";
         $results=$conn->query($sql1); 
		while($rs=$results->fetch_assoc()) { 
		?>
